...
// 매개변수로 작성된 url 패턴의 요청을 매핑하도록 지정함.
// -> web.xml에 작성하던 Serlvet 등록, 매핑 작업을 단축 시킴.
...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 전달 받은 값을 꺼내 변수에 저장
		String memberId = request.getParameter("memberId");
		String memberPwd = request.getParameter("memberPwd");
		
		// 3. Member 객체를 생성하여 id, pwd를 저장하여 한 번에 저장할 수 있게함.
		...
		try {
			...
			// 요청 성공
			if(loginMember != null) {
				//	Session : 서버쪽에 요청이 들어온 브라우저 마다 하나씩 생성되며
				//	브라우저가 종료 되거나, 세션이 만료 될 때 까지
				//	사라지지 않고 유지되면, 어떤 페이지에서든 접근 가능함.
				//	(Session은 Server에서 관리가 됨.)
				
				// 세션에 "loginMember" 키값으로 로그인된 정보를 속성으로 추가함.
				...
				// **** Cookie 생성 ****
				// 1. 로그인 성공 시 "아이디 저장" 체크 박스가 체크되었는지 따라 쿠키를 생성한다.
				...
				if(saveId != null) { // 아이디 저장이 체크가 된 경우
					...
					// 2. 쿠기가 유지될 수 있는 유효 기간 설정 (초단위)
					cookie.setMaxAge(60*60*24*7); // 유효 기간 7일 설정
					
					
				}else {
					
					cookie.setMaxAge(0); // 0초 인경우 쿠키가 삭제됨
					
					
				}
				
				// 3. 쿠키가 사용될 수 있는 유효한 디렉토리(주소)를 설정
				cookie.setPath("/"); // 최상위 디렉토리(주소) 를 나타냄
									// == 어떤 페이지 에서도 사용 가능
				
		
				// 4. response 객체에 쿠키 객체를 담아서 응답
