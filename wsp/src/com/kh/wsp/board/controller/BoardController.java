	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(command.equals("/list.do")) {
				...
				PageInfo pInfo = boardService.getPageInfo(currentPage, boardType);
				
				// 현재 페이지와, 한 페이지에 보여줄 게시글 수를 이용하여
			...
			// 게시글 등록(게시글 + 파일)
				...
				// 1_1. 전송 파일의 크기(용량) 제한 (byte 단위)
				int maxSize = 1024 * 1024 * 10; // 10MB
				
				// 1_2. 전송된 파일을 저장할 위치 경로를 지정
				// Servlet Container 경로 추출 -> WebContent폴더 위치
				String root = request.getSession().getServletContext().getRealPath("/");
				
				// uploadImages 폴더 위치 추출
				String filePath = root + "resources\\uploadImages";
				// C: \workspace\5_WebServer\wsp\WebContent\resources
				
				
				// 2. 파일명 변환 및 저장 작업
				/*
				 * 사용자가 업로드한 파일명을 그대로 저장하지 않는 것이 일반적임.
				 * 왜? 
				 * 	 - 같은 파일명으로 인한 덮어쓰기 방지
			...
			// 	  HttpServletRequest request를 이용하여 얻어옴.
			String memberId = ((Member)request.getSession().getAttribute("loginMember")).getMemberId();
			
			// 전달받은 문자 데이터를 Board VO 객체에 저장
			...
			Attachment temp = null; // 임시 참조 변수 저장
			while(files.hasMoreElements()) {
			//	files.hasMoreElements() : 다음 반복 접근할 요소가 있다면 true
			...
			if(mRequest.getFilesystemName(name)!= null) {
				...
				// img1 -> 0(썸네일), img2 -> 1, img 3 -> 2 , img4 -> 3
		...
		response.sendRedirect(path);
			...
			// 게시글 상세조회 Controller
				...
				path = "/board/list.do?type="+boardType;
				request.getSession().setAttribute("status", status);
				request.getSession().setAttribute("msg", msg);
				view = request.getRequestDispatcher(path);
