...
public class BoardDAO {
	...
	public BoardDAO() throws Exception{
		String fileName = 
				BoardDAO.class.getResource("/com/kh/wsp/sql/board/board-query.properties").getPath();
	...
	public List<Board> selectList(Connection conn, PageInfo pInfo) throws Exception{
		...
		try {
			// SQL 조건절에 사용할 값을 가공
			int startRow = 
					(pInfo.getCurrentPage()-1) * pInfo.getLimit()+1;
			
			int endRow 
				=	startRow + pInfo.getLimit() - 1;
	...
	public int insertAttachment(Connection conn, Attachment at) throws Exception{
		...
		try {
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, at.getPrarentBoarNo());
	...
	public Board updateView(Connection conn, int boardNo) throws Exception{
		...
		try {
			...
			if(rset.next()) {
				board = new Board(boardNo, rset.getString("BOARD_TITLE"), "BOARD_CONTENT", "BOARD_CATEGORY");
			}
