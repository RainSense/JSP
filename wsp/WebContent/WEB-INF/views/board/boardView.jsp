...
<body>
		...
		<div>
			...
			<div id="board-area">
               ...
               <% if(fList != null){ %>
				...
				<a href="delete.do?type=<%=board.getBoardType()%>&no=<%=board.getBoardNo()%>" class="btn btn-primary float-right">삭제</a>
					<!-- 삭제 버튼 클릭 시 해당 게시글 상태를 'N'으로 바꾸고 목록으로 돌아가기 -->
					 
					<a href="updateForm.do?no=<%=board.getBoardNo()%>&cp=<%=cp%>&type=<%=type%>" class="btn btn-primary float-right ml-1 mr-1">수정</a>
					<% } %>
					
					<a href="list.do?type=<%=type%>&cp=<%=cp%>" class="btn btn-primary float-right">목록으로</a>
				</div>
			</div>
	...
	$("#deleteBtn").on("click", function(){
		...
		if(confirm("정말 삭제 하시겠습니까?")){
			location.href = "delete.do?type=<%=board.getBoardType()%>&no=<%=board.getBoardNo()%>";
		}
